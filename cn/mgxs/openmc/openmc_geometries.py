...
def rectangular_lattice(
    ...
    lattice_pitch: float,
    fuel_or: float,
    fuel_material: openmc.Material | list[openmc.Material],  # type: ignore
    clad_ir: float,
    clad_or: float,
    ...
    if isinstance(fuel_material, openmc.Material):
        fuel_material: list[openmc.Material] = [fuel_material] * lattice_size**2  # type: ignore

    assert (
        len(fuel_material) == lattice_size**2
    ), "The number of fuel materials must be equal to lattice_size^2, or a single material must be provided"
    ...
    for fuel_pin_idx, material in enumerate(fuel_material):
        ...
        if clad_ir > fuel_or:
            gap_cell = openmc.Cell(region=gap_region)  # type: ignore
    ...
    lattice.universes = [
        fuel_pin_universes[i * lattice_size : (i + 1) * lattice_size] for i in range(lattice_size)
    ]

    lattice_prism = openmc.model.RectangularPrism(
        width=lattice_pitch * lattice_size,
        height=lattice_pitch * lattice_size,
